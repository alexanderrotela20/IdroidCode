 ...
 * @author Rosemoe
 */
public class ArSymbolInputView extends LinearLayout {

    private int textColor;
	private Typeface typeface;
    private CodeEditor editor;

    public ArSymbolInputView(Context context) {
        super(context);
        init();
    }
	

    public ArSymbolInputView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ArSymbolInputView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public ArSymbolInputView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init() {
    	//setBackgroundColor();
    	setBackgroundColor(getContext().getResources().getColor(com.ardev.idroid.R.color.color_surface));
        setOrientation(HORIZONTAL);
        setTextColor(getContext().getResources().getColor(com.ardev.idroid.R.color.color_onSurface));
	...
	for (int i = 0; i < getChildCount(); i++) {
            ((Button) getChildAt(i)).setTypeface(typefaceText);
        }
    ...
    public void addSymbols(String[] display, final String[] insertText) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            btn.setOnClickListener((view) -> {
                if (editor != null && editor.getEditable())
