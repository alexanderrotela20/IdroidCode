...
import com.ardev.component.view.BlinkLayout;
import com.ardev.component.view.DefaultView;
...
public class ViewCreator {
    ...
    public static View create(@NonNull String name, @NonNull Context ctx) {
        if (name.startsWith("com.ardev.idroid.")) return createDefault(ctx, name);
        View v = null;
    ...
    private static View _create(@NonNull String name, @NonNull Context ctx) {
        try {
            ...
            return (View) con.newInstance(ctx);
             ...
             MessageArray.getInstanse().logW("Al crear " + name + "：" + e1.getCause().toString());
        } catch (Exception e) {
        }
        return null;
    }
